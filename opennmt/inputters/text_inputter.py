...
class CharConvEmbedder(CharEmbedder):
  ...
  def __init__(self,
    ...
    self.conv.add(tf.keras.layers.Masking())
    self.conv.add(tf.keras.layers.Conv1D(
        num_outputs,
        kernel_size,
        strides=stride,
        padding="same",
...
class CharRNNEmbedder(CharEmbedder):
  ...
  def __init__(self,
    ...
    self.rnn.add(tf.keras.layers.Masking())
    self.rnn.add(tf.keras.layers.RNN(cell_class(num_units, dtype=dtype)))
